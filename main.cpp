...
if (pos == string::npos) {
    cerr << "Invalid node format: " << node << endl;
    continue;
}
...
// Protect against segfault if section is missing in slots
if (slots.find(section) == slots.end()) {
    cerr << "Section \"" << section << "\" not found in initialized slots.\n";
    continue;
}
...
public:
    ...
    fileNames.clear();  // 🔧 FIX: clear previous data
    ...
    void approvetable() {
        ...
        fileNames.clear();  // clear again to avoid reuse
    }
        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case 3:
                    // View Report
                    if(role == "student") {
                        cout<<"Not valid option for student\n";
                        break;
                    }
                    else graphProcessor.printColorMap();
                    break;
